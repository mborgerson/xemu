...
def gh_latest_release(owner: str, repo: str, pattern: re.Pattern) -> None | tuple[str, str]:
    """
...
def update_wrap(path: Path) -> None | UpdatedWrap:
    ...
    if sha.startswith(rev):
        log.info(f"%s already at %s (%s)", path.name, tag, sha)
        return None

    log.info(f"%s updated to %s (%s)", path.name, tag, sha)
...
def main():
    ...
    ap.add_argument("--manifest", "-m", action="store_true", default=False, help="Print JSON-formatted updated manifest")
    ap.add_argument("wraps", nargs="*", help="Which wraps to update, or all if unspecified")
    args = ap.parse_args()
